...
オリジナルデータでは，近畿大学の西野さんと又吉さんのスピーチからとりました．
...
./cache/JpSpeech/model_checkpoint
```init
CycleGAN-VC2
...
## 学習データの準備(オリジナルデータ)
今回はオリジナルデータとして，西野さんと又吉さんのスピーチをyoutubeからダウンロードして，準備しました．

西野さんと又吉さんのスピーチをwavに変換したのちに，10秒ずつ分割して学習データ・評価データを用意しました．

Google Driveでデータをやりとりする場合は，zipで圧縮したものをアップロードして，Colab上で回答した方がエラーも発生しづらくおすすめです．

以下のコマンドでzipの解凍ができます．


```python
!!unzip data/JpSpeech.zip > /dev/null
```
...
```python
!python preprocess_training.py --train_A_dir ./data/JpSpeech/train/matayoshi \
                               --train_B_dir ./data/JpSpeech/train/nishino \
                               --cache_folder ./cache/JpSpeech
```
...
ここでは「西野さんから又吉さんの声へ」，または「又吉さんの声から西野さんの声へ」の変換を学習しています．
...
# --resume_training_at ../cache/model_checkpoint/_CycleGAN_CheckPoint \
# 学習済みモデルの学習再開
!python train.py --logf0s_normalization ./cache/JpSpeech/logf0s_normalization.npz --mcep_normalization ./cache/JpSpeech/mcep_normalization.npz \
                 --coded_sps_A_norm ./cache/JpSpeech/coded_sps_A_norm.pickle --coded_sps_B_norm ./cache/JpSpeech/coded_sps_B_norm.pickle \
                 --model_checkpoint ./cache/JpSpeech/model_checkpoint/ --resume_training_at ./cache/JpSpeech/model_checkpoint/_CycleGAN_CheckPoint \
                 --validation_A_dir ./data/JpSpeech/eval/matayoshi/ --output_A_dir ./data/JpSpeech/converted_sound/matayoshi/ \
                 --validation_B_dir ./data/JpSpeech/eval/nishino/ --output_B_dir ./data/JpSpeech/converted_sound/nishino/
```
## test.pyの上書き
...
## 音質変換の実行
学習(約4日)が終わったので，出力された最終の重み`./cache/JpSpeech/model_checkpoint/_CycleGAN_CheckPoint`でtest.pyを実行し，ここでは「西野さんの声から又吉さんの声へ」の音質変換を実施します．


```python
# --test_A_dir ./data/JpSpeech/eval/matayoshi/ --output_A_dir ./data/test_converted_sound/matayoshi/ \
!python test.py --logf0s_normalization ./cache/JpSpeech/logf0s_normalization.npz --mcep_normalization ./cache/JpSpeech/mcep_normalization.npz \
                --test_B_dir ./data/JpSpeech/eval/nishino/ --output_B_dir ./data/test_converted_sound/nishino/ \
                --model_checkpoint ./cache/JpSpeech/model_checkpoint/_CycleGAN_CheckPoint
```
変換結果

<video width="640" height="400" controls>
  <source src="/video/nishino_matayoshi_vc.mp4" type="video/mp4">
  Your browser does not support the video tag.
</video> 

## まとめ
近畿大のスピーチをサンプルコードで，音質変換を実施しました．

変換は結構ひどかったです．

以下の点を改良すれば，よくなる可能性があります．
- 学習回数をさらに増やす
- データの見直し・質の統一
  - 又吉さんのスピーチはBGMとマイクの反響がない
  - 西野さんのスピーチは一部にBGMとマイクの反響がある
- モデルの変更
- モデルの設定の変更(バッチサイズ等)


日本語自体が変換するのに難しい可能性もありますので，日本語の音声データを発掘できたら，それでまず変換してみます．
